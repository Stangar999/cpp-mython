...
struct TestMethodBody : runtime::Executable {
    using Fn = std::function<ObjectHolder(Closure& closure, runtime::Context& context)>;
    Fn body;
    ...
    ObjectHolder Execute(Closure& closure, runtime::Context& context)  {
        if (body) {
...
//void MyTestNewInstance() {
//    runtime::DummyContext context;
//    Closure base_closure;
//    auto base_method_1 = [/*&base_closure, &context*/](Closure& closure, runtime::Context& ctx) {
//        //ASSERT_EQUAL(&context, &ctx);
//        //base_closure = closure;
//        return ObjectHolder::Own(runtime::Number{123});
//    };
//    vector<runtime::Method> base_methods;
//    base_methods.push_back(
//        {"__init__"s, {}, make_unique<TestMethodBody>(base_method_1)});
//    //base_methods.push_back({"test_2"s, {"arg1"s}, make_unique<TestMethodBody>(base_method_2)});

//    runtime::Class empty("Empty"s, {}, nullptr);
//    runtime::ClassInstance object{empty};
//    auto result = make_unique<NewInstance>(empty);
//    auto res = result.get();
//    //int a = 0;
...
void RunUnitTests(TestRunner& tr) {
    ...
    //RUN_TEST(tr, ast::MyTestNewInstance);
}
